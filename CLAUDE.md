...
Last updated: 2026-06-21 (rev 31)
...
## Pending / TODO (as of rev 31)

### Accounts — P&L and audit follow-ups
- **P&L / profit view (the payoff — not built):** Net Profit = Revenue − operating expenses; then − Remuneration (appropriation of profit) = Retained profit. Must sum **operating-kind expenses only** — the `categoryId→kind` filter in `ExpenseClient` is the reference pattern (remuneration is cash-out but never an operating expense).
