...
Last updated: 2026-09-24 (rev 40)
...
> **GiST EXCLUDE constraint**: ~~deferred to pre-launch test-data cleanup~~ ✅ APPLIED 2026-09-05 (`excl_booking_rooms_active_overlap`, status-scoped to confirmed/checked_in — the terminal test-data overlaps were never in scope, so the blocker assumption was stale). Structurally closes the concurrent-insert window in the create/add RPCs and replaces `uq_booking_room` (whose one-row-per-room-EVER assumption blocked released rooms from rejoining their booking — BK-1644).
>
...
> **rev 39 (2026-09-05)** — BK-1644: released rooms can rejoin their booking. Root cause was `uq_booking_room` (UNIQUE booking_id+room_id from the 2026-05-08 foundation — "one row per room per booking, EVER", predating the statuses that make rows terminal mid-booking), discovered when the hardened add_room_to_booking probe failed on it. Replaced by ONE designed constraint, `excl_booking_rooms_active_overlap` (GiST, status-scoped): keeps duplicate protection in its correct overlap form, permits rejoin + split stays, and structurally closes the create/add concurrent-insert window. Dependency sweep confirmed nothing relied on one-row-per-room. `add_room_to_booking` hardened in the same pass (span guard, server-derived nights — the LAST client-trusted nights value — and the guard error now names the conflicting booking + actual covered range). Client: Add Room dialog defaults to today → latest blocking-row checkout (the old original-span default made the guard correctly block the released room's own consumed nights); guard errors surface verbatim. Migration `2026-09-05-add-room-rejoin.sql`.
...
> **rev 40 (2026-09-24)** — BK-1713 + BK-1820: the **persist-before-RPC doctrine** is now general. **RULE: every figure the checkout RPC's maths must judge — override audit (Step 0.5, rev 37), in-modal payments (BK-1713: `recordPayment` returns `Promise<boolean>`, awaited pre-RPC, pay fields cleared on persist so a retry can't re-charge), and in-modal extra charges (BK-1820: `checkoutNormal` Step 0.6; folded into `checkoutWithOverride`'s Step 0.5) — is written to the DB BEFORE the RPC call.** A post-RPC write means step 3.5's overpayment auto-refund, the balance guard, and `chk_paid_not_exceed_total` all judge stale figures: BK-1820's shape (extension night + same-evening early departure + in-modal discount + payment) auto-refunded ৳800 the hotel meant to keep. Three companion changes: (1) `deny_refund` now RAISES for `pre_adjusted` refunds (`2026-09-24-deny-refund-pre-adjusted-guard.sql`) — their negative payment is written at refund CREATION (constraint-forced), so a deny left a phantom cash outflow; the only legal exits are disburse, or keep the money via a matching extra charge (literal deny-reversal alone would violate `chk_paid_not_exceed_total`). (2) Both checkout modals grew a **"Charge the unused night(s)" toggle** (shown when the picked departure produces a deduction, reason MANDATORY): the waived deduction travels as an extra charge (`Unused night(s) charged — {reason}`), so an agreed extension night isn't silently refunded as "early departure" — no schema change; the pre-RPC extra-charge write is what makes it visible to the refund maths. (3) FrontDesk's discount validator unified to the Bookings rule (cap = billable total, NOT remaining-after-payment — the old cap silently rejected the discount once an in-modal payment covered the bill, which is how BK-1820's ৳400 dropped on retry). Regression probe (ROLLBACK-wrapped BK-1820 replay, in the migration file) PASSED: zero refund rows, effective total = discounted figure = paid. BK-1820 data corrected 2026-09-24 (variant B: extra +1,200, reversal payment +800, discount 400 — drawer and books agree).
...
#### Known Behaviour (DELIBERATE, not a bug) — a checkout discount up to the outstanding balance clears it; write-offs are financially invisible
Read this FIRST when investigating "missing revenue" or "checkout bypassed the balance guard".

Current state (2026-08-15, decision by Arif): a "More Discount" up to the outstanding balance clears it and allows checkout **without** an admin override — enforced consistently at BOTH layers. Client: the discount is subtracted into `finalPayable` before the balance branch evaluates. Server: `checkout_booking` runs `assert_checkout_allowed` **AFTER** step 3.6 writes `additional_discount_*` (placement deliberately REVERSED on 2026-08-15 — it briefly sat before 3.6 via `2026-08-14-checkout-balance-guard.sql`, which closed this route; see `2026-08-15-checkout-guard-after-discount.sql`). The guard still hard-blocks any checkout where a balance REMAINS after the discount: staff get a clear error; only an admin with a mandatory recorded reason can override (`p_override` → `auth.uid()`→`profiles.role`, fail-closed on NULL). Guardrails on the discount itself: capped at 100% of the billable total (`validateAndBuildDiscount`), fields collapsed behind the amber "Apply Discount" disclosure button (Add Payment is the prominent solid-green path — accidental-entry fix for the incident below), the reason is recorded, and `additional_discount_by` names the applier. The per-room checkout RPC has no discount parameter and always requires a settled balance. History: nine bookings totalling ৳33,900 were zeroed via the then-always-visible discount field between 2026-07-31 and 2026-08-02 (accepted, left as-is) — that incident drove the disclosure UI and the server-side guard.
...
| 2026-09-05 | `2026-09-05-add-room-rejoin.sql` | uq_booking_room → excl_booking_rooms_active_overlap (GiST, status-scoped; rejoin + split stays allowed, concurrency window closed); add_room_to_booking derives nights + span guard + informative error | ✅ Applied |
...
| 2026-09-24 | `2026-09-24-deny-refund-pre-adjusted-guard.sql` | deny_refund RAISES for pre_adjusted (auto-overpayment) refunds — negative payment written at creation makes deny a phantom cash outflow; disburse or extra-charge instead. UPDATE row-count guard added (BK-1820 finding 3) | ✅ Applied |
