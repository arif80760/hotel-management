-- 2026-06-08 — Propagate checked_out_early to the BOOKING-level status
--
-- WHY:
--   Rooms were correctly marked checked_out_early on an early departure, but the
--   booking itself was always promoted to plain checked_out by every checkout
--   path. So a booking that left early read as a normal checkout — invisible to
--   any query keyed on booking.status. This makes checked_out_early a proper
--   sibling of checked_out everywhere the status is SET.
--
-- RULE (uniform): when a booking has no active rooms left (none confirmed /
--   checked_in), it is checked_out_early if ANY of its rooms is checked_out_early,
--   otherwise checked_out.
--
-- NO MONEY MATH CHANGES. Deductions, refunds, overpayment checks, total
--   recomputation are all byte-for-byte unchanged. Only status labels and the
--   timestamp condition change. Every `room -> 'cleaning'` write is left as-is
--   (cleaning-lifecycle cleanup is a separate, deferred item).
--
-- Functions changed:
--   1. fn_stamp_booking_timestamps — stamp checked_out_at on either terminal state
--   2. checkout_booking            — step 5 early-aware
--   3. checkout_booking_room       — room marked early + booking promotion early-aware
--   4. cancel_booking_room         — step 8 status derivation early-aware
--
...
BEGIN
  ...
  IF NEW.status IS DISTINCT FROM OLD.status THEN
    ...
    -- checked_out_early is a sibling of checked_out. Without it here, an early
    -- checkout would never record its checked_out_at timestamp.
    IF NEW.status IN ('checked_out','checked_out_early') AND OLD.checked_out_at IS NULL THEN
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 2. checkout_booking  (booking-level checkout — the Front Desk path)
-- ─────────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 3. checkout_booking_room  (per-room checkout — Bookings page)
-- ─────────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 4. cancel_booking_room  (per-room cancel / per-room early checkout)
-- ─────────────────────────────────────────────────────────────────────────────
